...
- Strict parsing initially produced 8 false positives across three real docs (`HANDOVER_WORKFLOW_DISCUSSION.md`, `POST_GRILL_1.md`, `ALFRED_HANDOVER_15.md`) — every one of them inside markdown code spans documenting the tag grammar itself (e.g. `` `[future-doc:]` `` as a label). Without code-span awareness, every doc that *describes* the parser would fail the parser. Resolved by adding narrow code-span skipping for triple-backtick fenced blocks plus inline code spans of any backtick-run length (using a backreference so `` `` `[future-doc:]` `` `` is recognised as a single double-backtick span around a literal token). The first iteration only handled single-backtick spans and the post-mortem itself reintroduced the failure via a double-backtick example; the follow-up regex generalisation closed that gap. Explicitly not a generalised markdown parser.
...
- *Markdown code-span skipping in the parser.* What changed: parser now skips text inside triple-backtick fenced blocks plus inline code spans delimited by any matching backtick-run length (`_FENCED_BLOCK_RE`, `_INLINE_CODE_RE`), including double-backtick spans around a single-backtick token. Why: without it, every doc that documents the tag grammar in prose would be flagged as malformed; this is targeted, not "generalised markdown parsing." Approved by: executor judgment, recorded here. Considered the alternative of editing 3 docs to use a non-bracket form; rejected as it would amputate the docs' ability to discuss their own grammar.
