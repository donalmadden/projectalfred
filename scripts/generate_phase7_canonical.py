"""Generate a refreshed canonical Phase 7 handover from the latest repo state.

This follows the same planner pipeline as ``generate_phase7.py`` but seeds the
planner with the archived ``docs/archive/ALFRED_HANDOVER_6_old.md`` artifact and
only writes the canonical ``docs/canonical/ALFRED_HANDOVER_6.md`` path after the
...
DISPLAY_TITLE = "Phase 7 Canonical Refresh"
SOURCE_FILENAME = f"{EXPECTED_HANDOVER_ID}_old.md"
...
SOURCE_PATH = _resolve_manifest_doc_path(
    ...
    kind="archived_handover_snapshot",
    fallback_relative_path=Path("docs/archive") / SOURCE_FILENAME,
)
...
_DOC_PATH_OVERRIDES = {
    ...
    f"docs/{SOURCE_FILENAME}": SOURCE_PATH.relative_to(REPO_ROOT).as_posix(),
    f"docs/{FAILED_FILENAME}": FAILED_OUTPUT_PATH.relative_to(REPO_ROOT).as_posix(),
}

SPRINT_GOAL = (
    "Refresh the canonical Phase 7 handover so "
    "`docs/canonical/ALFRED_HANDOVER_6.md` reflects the latest repository state, "
    "completed grounding work, current repo-growth rules, real git history, and "
    "any remaining Phase 7 tasks without preserving stale claims from the "
    "historical snapshot."
)
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate and validate a refreshed canonical Phase 7 handover.",
    )
    parser.add_argument(
        "--source",
        type=Path,
        ...
        help=(
            "Historical handover to refresh "
            "(default: docs/archive/ALFRED_HANDOVER_6_old.md)"
        ),
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(default: docs/archive/ALFRED_HANDOVER_6_FAILED_CANDIDATE.md)"
        ),
    )
    parser.add_argument(
        ...
        default="summary",
        help=(
            "How much of the old handover to pass into the planner. "
            "Default: summary"
        ),
    )
...
def _is_citable_doc_reference(path: str) -> bool:
    ...
    return entry.citable if entry is not None else True
...
def _archive_doc_label(path: str) -> str:
    ...
    if filename == SOURCE_FILENAME:
        return "the archived Phase 7 historical handover snapshot"
    if filename == FAILED_FILENAME:
        return "the archived failed Phase 7 candidate"
...
def load_historical_context(
    ...
    *,
    ...
    if mode == "full":
        return (
            "Update the historical canonical handover below so it matches the live "
            "repository today. Preserve the handover identity (`ALFRED_HANDOVER_6`) "
            "but prefer repo facts, validators, and git history over any stale prose. "
            "Archive-only docs are continuity inputs, not citable reference docs.\n\n"
            "Historical source: archived Phase 7 handover snapshot (continuity only).\n\n"
    ...
    parts: list[str] = [
        "Use this historical summary for continuity only. Treat repo facts, validator "
        "findings, reference-doc checks, and current git history as more authoritative "
        "than any stale content from the archived handover. Do not cite archive-only "
        "docs in the new handover's `Reference Documents` block.",
        "Historical source: archived Phase 7 handover snapshot (continuity only).",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    retrieval_query = (
        f"{SPRINT_GOAL} {EXPECTED_HANDOVER_ID} canonical refresh {source_path.name}"
    )
    ...
    if source_path.is_file():
        ...
        print(f"WARNING: source handover not found at {source_path}; refreshing without it")
